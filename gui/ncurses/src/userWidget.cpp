...
userWidget::userWidget(samUser *inSamUser, unsigned int inUserIndex) : baseUserWidget(inSamUser, inUserIndex)
{
    ...
    resetLabel();
    this->menuItem = new_item(this->label.c_str(), "");
}
...
void userWidget::resetLabel()
{
    ...
    this->label = stringPrintf("%s%s %s%s%s", user->getUserName().c_str(), fullName.c_str(), msAccount.c_str(), isBlank.c_str(), isDisabled.c_str());
}

userWidget::~userWidget() {
    std::cout << "Destroying menu item" << std::endl;
    free_item(this->menuItem);
}
