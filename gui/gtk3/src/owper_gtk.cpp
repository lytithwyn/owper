...
void owper_gtk::loadGUI() {
    ...
    vboxMain = gtk_box_new(GTK_ORIENTATION_VERTICAL, 5);
    ...
    hboxHivePath = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
    ...
    vboxUsers = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);
    gtk_container_add(GTK_CONTAINER(scrollwinUsers), vboxUsers);

    //commands across the bottom
    hboxCommands = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
